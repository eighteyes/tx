...
✅ **Unique Identification**: Each spawned mesh instance has a unique suffix based on its task
✅ **Task Context**: The UID makes it immediately clear what task a session is for
✅ **Deterministic**: Same task always generates the same UID
✅ **Compact**: Only 4 characters - easy to type and remember
✅ **No Breaking Changes**: Without `-i` flag, behavior is unchanged
