...
Write `oracle-result.yaml` to workspace after validation. FSM gates on this file.

```yaml
# $workspace/oracle-result.yaml
...
FSM reads `verdict` field: `pass` routes to compress, `fail` routes to render_revise.
...
- Write `oracle-result.yaml` at end of every validation. FSM gates on this file.
- Pass if thorough check finds no violations. Look first.
