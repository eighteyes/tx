...
- Route based on verdict: approved → assemble narrator context → narrator, violations → sim-chars
...
```bash
$TX_ROOT/meshes/narrative-engine/scripts/assemble-narrator.sh "{workspace}" "{game_path}" "{campaign_path}"
```
