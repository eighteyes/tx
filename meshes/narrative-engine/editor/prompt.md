...
- Run mechanical lint (`mechanical-lint.sh`) + 4 parallel lint Tasks (patterns, temporal, metaphor — sonnet; facts — haiku)
...
**For facts Task (haiku):**
- Generate character briefs: `$SCRIPTS/character-brief.sh {character_id} {campaign_path} {game_path} --mode=task` — run once per character present
- Read scene script: `$SCRIPTS/read-state.sh {workspace} scene-script` (may reuse from temporal gather)
...
Read $TX_ROOT/meshes/narrative-engine/refs/lint-patterns.md for detection rules.
...
Read $TX_ROOT/meshes/narrative-engine/refs/lint-temporal.md for detection rules.
...
Read $TX_ROOT/meshes/narrative-engine/refs/lint-metaphor.md for detection rules.
...
Use model: haiku for this Task — it is extraction work, not creative judgment.

This Task requires character entity data. Generate merged briefs before firing:

```bash
$SCRIPTS/character-brief.sh {character_id} {campaign_path} {game_path} --mode=task
```

Run once per character present in the scene. Collect the output. Task mode strips psychology, backstory, sexuality — facts lint needs appearance, traits, voice, life concerns only.

```
You check factual consistency between prose and canonical character data. You see ONLY the prose text, filtered character briefs, and scene_script.

Read $TX_ROOT/meshes/narrative-engine/refs/lint-facts.md for detection rules.

## Prose
{full content of prose-draft.md}

## Character Briefs
{merged character-brief.sh output for each character present}

## Scene Script
{scene_script.yaml content — for claim-scene verification}

## Task
1. Read the lint-facts ref for all detection rules and workflow
2. Build fact index from character briefs (physical, habits, voice)
3. Extract every factual claim from prose
4. Compare claims to canonical entity data
5. Check within-turn consistency and claim-scene consistency
6. Write your violations to {workspace}/violations-facts.yaml
...
**Facts fixes:**
- Entity contradiction (wrong eye color, wrong coffee order): replace with canonical value from entity data
- Entity deviation without acknowledgment: add one interior beat noting the departure, or correct to canon
- Phantom event (debrief references unrendered scene): either add the missing scene moment or revise the debrief claim
- Self-contradiction (same fact described differently within turn): pick the canonical version, fix the other
