...
Then send a "new campaign" message to `narrative-engine/entry` — or use the calibrator's worldbuilder mode to tune any artifact before playing.
...
Run the calibrator (send "new game" to `narrative-engine/entry`) for a guided 9-phase extraction loop that builds all game artifacts from conversation. Or copy an example game and use worldbuilder mode to reshape it.
