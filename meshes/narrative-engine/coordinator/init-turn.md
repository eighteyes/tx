...
```bash
$TX_ROOT/meshes/narrative-engine/scripts/init-workspace.sh --stamp-action "$RAW_ACTION" --verbose
```
...
REGISTER_GUIDE=$($SCRIPTS/read-state.sh {game_path} author --key="register_guides.{scene_type}" 2>/dev/null)
...
```bash
$TX_ROOT/meshes/narrative-engine/scripts/stamp-decomposition.sh \
  "{workspace}" {turn} \
