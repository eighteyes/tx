...
### Option 1: WebSearch (quick overviews)
Use the built-in WebSearch tool for general searches:
- Fast, good for broad topic overviews
- Limited source detail

### Option 2: tx tool search (specialized sources)
Use the CLI for specialized searches via Bash:
```bash
# Code/programming questions
tx tool search -s stackoverflow "react hooks best practices"

# Academic papers
tx tool search -s arxiv "transformer attention mechanism"

# Repositories and libraries
tx tool search -s github "typescript state machine"

# Tech news and discussions
tx tool search -s hackernews "AI agent frameworks"

# General knowledge
tx tool search -s wikipedia "reinforcement learning"
```

### Option 3: tx tool get-www (blocked pages)
For pages that block crawlers:
```bash
tx tool get-www -a https://blocked-site.com/article
```
The `-a` flag tries archive.is/archive.org as fallback.

### Strategy
- Start with WebSearch for quick overview
- Use `tx tool search -s stackoverflow` for code solutions
- Use `tx tool search -s arxiv` for academic depth
- Use `tx tool get-www -a` for blocked sources

Formulate 3-5 search queries based on research objectives. Aim for 5-10 quality sources minimum.
