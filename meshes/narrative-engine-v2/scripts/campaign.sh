...
condition_usage() {
  cat <<'EOF'
condition domain — mutable temporal states (NRE, grief, trauma, obsession, etc.)

Conditions live inside entity files (characters or bonds) under `conditions:`.
...
set: upsert — creates if new, replaces specified fields if existing.
  Top-level fields: --type, --phase, --onset, --intensity, --expected-arc
  Nested fields: --field=manifestations.physical --value="new text"
  Multiple nested fields: repeat --field/--value pairs
...
list: list all conditions on entity. --active excludes resolved. --type filters.

resolve: mark condition as resolved, record what it became.
EOF
  exit 1
}
...
condition_set() {
  ...
  # Collect --field/--value pairs before parse_options consumes them
  local -a field_keys=()
  local -a field_vals=()
  ...
  while [[ $# -gt 0 ]]; do
    if [[ "$1" == "--field="* ]]; then
      field_keys+=("${1#--field=}")
      shift
      if [[ $# -gt 0 && "$1" == "--value="* ]]; then
        field_vals+=("${1#--value=}")
        shift
      elif [[ $# -gt 1 && "$1" == "--value" ]]; then
        shift
        field_vals+=("$1")
        shift
      else
        die "--field must be followed by --value"
      fi
    elif [[ "$1" == "--field" ]]; then
      shift
      local fk="$1"; shift
  ...
  # Ensure conditions map exists
  local has_conditions
  has_conditions=$(yq '.conditions // "null"' "$entity_file")
  if [[ "$has_conditions" == "null" ]]; then
    yq -i '.conditions = {}' "$entity_file"
  fi

  # Check if condition exists
  local exists
  exists=$(yq ".conditions.\"$id\" // \"null\"" "$entity_file")

  if [[ "$exists" == "null" ]]; then
    # CREATE — new condition
    local type=$(opt "type")
    local phase=$(opt "phase")
    local onset="${turn}"  # Default onset to current turn
    [[ -n "$(opt 'onset')" ]] && onset=$(opt "onset")
    local intensity=$(opt "intensity")
    local expected_arc=$(opt "expected-arc")
    local pace=$(opt "pace")

    # Derive onset_day from story_day
    local onset_day="${STORY_DAY:-0}"
    [[ -n "$(opt 'onset-day')" ]] && onset_day=$(opt "onset-day")
    ...
    yq -i ".conditions.\"$id\" = {
      ...
      \"onset\": $onset,
      \"onset_day\": $onset_day,
      \"last_phase_day\": $onset_day,
      \"pace\": \"${pace:-medium}\",
      \"intensity\": \"${intensity:-unset}\",
      \"active\": true,
      \"last_updated\": $turn
    }" "$entity_file"

    [[ -n "$expected_arc" ]] && yq -i ".conditions.\"$id\".expected_arc = $(echo "$expected_arc" | yq -n 'env(arc)' --arg arc "$expected_arc")" "$entity_file" 2>/dev/null || true

    # Initialize manifestations if any shorthand provided
    local phys=$(opt "physical")
    local cog=$(opt "cognitive")
    local behav=$(opt "behavioral")
    local spch=$(opt "speech")
    if [[ -n "$phys" || -n "$cog" || -n "$behav" || -n "$spch" ]]; then
      yq -i ".conditions.\"$id\".manifestations = {}" "$entity_file"
      [[ -n "$phys" ]] && yq -i ".conditions.\"$id\".manifestations.physical = strenv(v)" --arg v "$phys" "$entity_file" 2>/dev/null || \
        yq -i ".conditions.\"$id\".manifestations.physical = \"$phys\"" "$entity_file"
      [[ -n "$cog" ]] && yq -i ".conditions.\"$id\".manifestations.cognitive = \"$cog\"" "$entity_file"
      [[ -n "$behav" ]] && yq -i ".conditions.\"$id\".manifestations.behavioral = \"$behav\"" "$entity_file"
      [[ -n "$spch" ]] && yq -i ".conditions.\"$id\".manifestations.speech = \"$spch\"" "$entity_file"
    fi

    # Initialize evolution if any shorthand provided
    local advances=$(opt "advances")
    local regresses=$(opt "regresses")
    local resolves=$(opt "resolves-into")
    if [[ -n "$advances" || -n "$regresses" || -n "$resolves" ]]; then
      yq -i ".conditions.\"$id\".evolution = {}" "$entity_file"
      if [[ -n "$advances" ]]; then
        # Convert CSV to YAML array
        local adv_yaml="["
        IFS=',' read -ra ADV <<< "$advances"
        for a in "${ADV[@]}"; do
          adv_yaml+="\"$(echo "$a" | xargs)\","
        done
        adv_yaml="${adv_yaml%,}]"
        yq -i ".conditions.\"$id\".evolution.advances = $adv_yaml" "$entity_file"
      fi
      if [[ -n "$regresses" ]]; then
        local reg_yaml="["
        IFS=',' read -ra REG <<< "$regresses"
        for r in "${REG[@]}"; do
          reg_yaml+="\"$(echo "$r" | xargs)\","
        done
        reg_yaml="${reg_yaml%,}]"
        yq -i ".conditions.\"$id\".evolution.regresses = $reg_yaml" "$entity_file"
      fi
      [[ -n "$resolves" ]] && yq -i ".conditions.\"$id\".evolution.resolves_into = \"$resolves\"" "$entity_file"
    fi

    echo "OK: condition '$id' created on $(basename "$entity_file")"
  else
    # MUTATE — update existing condition, only specified fields
    local type=$(opt "type")
    local phase=$(opt "phase")
    local intensity=$(opt "intensity")
    local expected_arc=$(opt "expected-arc")
    ...
    # PACE ENFORCEMENT — gate phase transitions by story-day elapsed
    if [[ -n "$phase" ]]; then
      local current_phase
      current_phase=$(yq ".conditions.\"$id\".phase" "$entity_file")
      if [[ "$phase" != "$current_phase" ]]; then
        local cond_pace
        cond_pace=$(yq ".conditions.\"$id\".pace // \"medium\"" "$entity_file")
        local last_phase_day
        last_phase_day=$(yq ".conditions.\"$id\".last_phase_day // 0" "$entity_file")
        local threshold=${PACE_THRESHOLDS[$cond_pace]:-7}
        local elapsed=$(( ${STORY_DAY:-0} - last_phase_day ))

        if [[ $elapsed -lt $threshold ]]; then
          echo "BLOCKED: phase transition '$current_phase' → '$phase' on '$id'"
          echo "  pace=$cond_pace requires $threshold+ story-days, only $elapsed elapsed (day $last_phase_day → day ${STORY_DAY:-0})"
          echo "  Intensity and manifestations can still change. Phase cannot advance yet."
          # Still allow other mutations (intensity, manifestations) — just skip phase
          phase=""
        else
          # Phase change allowed — update last_phase_day
          yq -i ".conditions.\"$id\".last_phase_day = ${STORY_DAY:-0}" "$entity_file"
        fi
      fi
    fi

    [[ -n "$type" ]] && yq -i ".conditions.\"$id\".type = \"$type\"" "$entity_file"
    [[ -n "$phase" ]] && yq -i ".conditions.\"$id\".phase = \"$phase\"" "$entity_file"
    [[ -n "$intensity" ]] && yq -i ".conditions.\"$id\".intensity = \"$intensity\"" "$entity_file"
    [[ -n "$expected_arc" ]] && yq -i ".conditions.\"$id\".expected_arc = \"$expected_arc\"" "$entity_file"
    [[ -n "$pace" ]] && yq -i ".conditions.\"$id\".pace = \"$pace\"" "$entity_file"

    # Manifestation shorthand
    local phys=$(opt "physical")
    local cog=$(opt "cognitive")
    local behav=$(opt "behavioral")
    local spch=$(opt "speech")
    # Ensure manifestations exists if any shorthand used
    if [[ -n "$phys" || -n "$cog" || -n "$behav" || -n "$spch" ]]; then
      local has_manif
      has_manif=$(yq ".conditions.\"$id\".manifestations // \"null\"" "$entity_file")
      [[ "$has_manif" == "null" ]] && yq -i ".conditions.\"$id\".manifestations = {}" "$entity_file"
    fi
    [[ -n "$phys" ]] && yq -i ".conditions.\"$id\".manifestations.physical = \"$phys\"" "$entity_file"
    [[ -n "$cog" ]] && yq -i ".conditions.\"$id\".manifestations.cognitive = \"$cog\"" "$entity_file"
    [[ -n "$behav" ]] && yq -i ".conditions.\"$id\".manifestations.behavioral = \"$behav\"" "$entity_file"
    [[ -n "$spch" ]] && yq -i ".conditions.\"$id\".manifestations.speech = \"$spch\"" "$entity_file"
    ...
    if [[ -n "$advances" || -n "$regresses" || -n "$resolves" ]]; then
      local has_evol
      has_evol=$(yq ".conditions.\"$id\".evolution // \"null\"" "$entity_file")
      [[ "$has_evol" == "null" ]] && yq -i ".conditions.\"$id\".evolution = {}" "$entity_file"
      if [[ -n "$advances" ]]; then
        local adv_yaml="["
        IFS=',' read -ra ADV <<< "$advances"
        for a in "${ADV[@]}"; do
    ...
    yq -i ".conditions.\"$id\".last_updated = $turn" "$entity_file"

    echo "OK: condition '$id' mutated on $(basename "$entity_file")"
  fi

  # Apply arbitrary --field/--value pairs (nested paths)
  for i in "${!field_keys[@]}"; do
    local fpath="${field_keys[$i]}"
    local fval="${field_vals[$i]}"
    yq -i ".conditions.\"$id\".$fpath = \"$fval\"" "$entity_file"
  done
}
...
condition_get() {
  ...
  local exists
  exists=$(yq ".conditions.\"$id\" // \"null\"" "$entity_file")
  [[ "$exists" == "null" ]] && die "Condition '$id' not found on $(basename "$entity_file")"

  yq ".conditions.\"$id\"" "$entity_file"
}
...
condition_list() {
  ...
  if [[ "$active" == "true" && -n "$type_filter" ]]; then
    yq ".conditions | to_entries | map(select(.value.active == true and .value.type == \"$type_filter\")) | from_entries" "$entity_file"
  elif [[ "$active" == "true" ]]; then
    yq '.conditions | to_entries | map(select(.value.active == true)) | from_entries' "$entity_file"
  elif [[ -n "$type_filter" ]]; then
    yq ".conditions | to_entries | map(select(.value.type == \"$type_filter\")) | from_entries" "$entity_file"
  else
    yq '.conditions' "$entity_file"
  fi
}
...
condition_resolve() {
  ...
  local id=$(opt_or_die "id")
  local turn=$(opt_or_die "turn")
  local resolved_into=$(opt "resolved-into")

  local exists
  exists=$(yq ".conditions.\"$id\" // \"null\"" "$entity_file")
  [[ "$exists" == "null" ]] && die "Condition '$id' not found on $(basename "$entity_file")"

  yq -i ".conditions.\"$id\".active = false" "$entity_file"
  yq -i ".conditions.\"$id\".resolved_turn = $turn" "$entity_file"
  [[ -n "$resolved_into" ]] && yq -i ".conditions.\"$id\".resolved_into = \"$resolved_into\"" "$entity_file"
  yq -i ".conditions.\"$id\".last_updated = $turn" "$entity_file"

  echo "OK: condition '$id' resolved at turn $turn"
}
...
# Derive story_day from arc.yaml (used by condition pace enforcement)
STORY_DAY=""
if [[ -f "$ARC_FILE" ]]; then
  STORY_DAY=$(yq '.story_day // 0' "$ARC_FILE" 2>/dev/null)
fi

# Pace thresholds: minimum story-days between phase transitions
# instant=0, fast=2, medium=7, slow=30, glacial=90
declare -A PACE_THRESHOLDS=(
  [instant]=0
  [fast]=2
  [medium]=7
  [slow]=30
