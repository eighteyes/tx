...
|------------------------------------------------------------------|------------------|------------------------------------------------------------------|
| `resolution.yaml` + `dramaturg-notes.yaml` + `threads.yaml`     | **Completion**   | All output written. Send routing message only.                   |
| `entropy-tables.yaml` + `resolution.yaml` (no dramaturg-notes)  | **Step 5**       | Tables merged and resolved. Run arc satisfaction + write notes.  |
| `dramaturg-guidance.yaml` (no entropy-tables)                    | **Step 3**       | Guidance written. Send message to entropy-gen.                   |
...
```yaml
---
to: narrative-engine-v2/entropy-gen
from: narrative-engine-v2/dramaturg
headline: "Guidance ready — generate entropy tables"
---
...
After receiving completion from entropy-gen, read the merged results:
- `{workspace}/entropy-tables.yaml`
- `{workspace}/resolution.yaml`
...
4. **World event count** — scale by `world_intervention_level`:
   ...
   - **minimal**: 0-1 world events. Ambient only. Zero is valid.
5. **CHAOS EVENT ENFORCEMENT** — Each chaos event must have 7-10 root manifestations, each with exactly 4 subtable entries: 3 register-toned (each a DIFFERENT register — no duplicates within a root), 1 thematic. ONE chaos world event must be thematically connected to the story. HARD GATE.
6. **THEMATIC EVENT ENFORCEMENT** — Each thematic event must have 3-7 manifestations. HARD GATE.

**If unsatisfied** → send retry message to entropy-gen with specific domains to regenerate:

```yaml
---
to: narrative-engine-v2/entropy-gen
from: narrative-engine-v2/dramaturg
headline: "Regenerate specific domains"
---
workspace: {workspace_path}
retry: true
domains: [{list of domains to regenerate}]
issues: "{what's wrong}"
```
...
Read all `{workspace}/entropy_tables/dramaturg-*.yaml` files and synthesize.
...
Gather option seeds from all character analyses in `entropy_tables/dramaturg-*.yaml` into a unified list in dramaturg-notes.yaml. Door-valence collisions from gravity become additional option seeds.
...
**dramaturg-guidance.yaml** — action_weight, world_intervention_level, entropy_mode, distribution_shape, thread_allocation, guidance, emotional_momentum, trajectory_status, recently_saturated_motifs, chaos_register, pov_character, characters_present
**dramaturg-notes.yaml** — outcome_shapes{}, guidance, emotional_momentum, option_seeds[], ending (MAX 60 LINES)
...
- **You do NOT write entropy-tables.yaml or resolution.yaml.** Entropy-gen owns those.
