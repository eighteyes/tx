/**
 * Test 10: Graded Mesh Flow
 *
 * Integration test verifying the `graded: true` mesh config triggers
 * quality hooks through the dispatcher → hooks → quality stack pipeline.
 */
...
describe('Graded Mesh Flow', () => {
  ...
  beforeEach(() => {
    ...
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'tx-v4-graded-'));
    ...
    fs.mkdirSync(path.join(meshesDir, 'agents', 'graded-test'), { recursive: true });
  ...
  test('graded: true expands to quality hooks via dispatcher', async () => {
    // Create mesh config with graded: true
    const meshConfig = {
      mesh: 'graded-test',
      description: 'Test mesh with quality gates',
      agents: [
        { name: 'worker', model: 'haiku', prompt: 'meshes/agents/graded-test/worker.md' }
      ],
      entry_point: 'worker',
      graded: true // Should expand to all quality gates
    };
    fs.writeFileSync(
      path.join(meshesDir, 'configs', 'graded-test.json'),
      JSON.stringify(meshConfig, null, 2)
    );

    // Create test worker prompt
    const workerPrompt = `# Graded Test Worker\nYou are a test worker.`;
    fs.writeFileSync(
      path.join(meshesDir, 'agents', 'graded-test', 'worker.md'),
      workerPrompt
    );
    ...
    assert.ok(true, 'Dispatcher started and can load graded mesh config');
  });

  test('graded: true with specific gates expands correctly', async () => {
    // Create mesh config with specific gates
    const meshConfig = {
      mesh: 'specific-gates',
      ...
      entry_point: 'worker',
      graded: ['checklist', 'rubric'] as GateType[]  // Only specific gates
    };
    fs.writeFileSync(
    ...
    assert.ok(true, 'Graded config with specific gates is valid');
  });
  ...
  test('quality:preflight hook populates context', async () => {
    ...
    assert.strictEqual(context.qualityMaxIterations, 3, 'qualityMaxIterations should default to 3');
