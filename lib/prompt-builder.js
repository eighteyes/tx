...
class PromptBuilder {
   ...
   */
  static build(mesh, agent, task = null) {
    try {
      ...
      if (!PromptBuilder._isOrchestrator(agentDir)) {
        workflow = PromptBuilder._getWorkflow(mesh, agent || mesh);
      }
  ...
  /**
   * Generate conditional next agent instructions based on mesh workflow topology
   */
  static _getNextAgentInstructions(mesh, currentAgent) {
    try {
      ...
      if (!fs.existsSync(configPath)) {
        return 'No workflow configuration found.';
      }

      const config = fs.readJsonSync(configPath);
      const agents = config.agents || [];
      const workflowType = config.type || 'linear';

      // Find current agent index (handle both "mesh/agent" and "agent" format)
      const currentAgentFull = currentAgent.includes('/')
        ? currentAgent  // Already in mesh/agent format
        : `${mesh}/${currentAgent}`;  // Need to add mesh prefix
      const currentIndex = agents.findIndex(a =>
        a === currentAgentFull || a === currentAgent
      );

      if (currentIndex === -1) {
        return 'Agent not found in workflow configuration.';
      }

      const isLastAgent = currentIndex === agents.length - 1;
      const isIterative = workflowType === 'iterative';

      // For linear workflow, only route to next agent or complete
      if (workflowType === 'linear' || workflowType === 'sequential') {
        if (!isLastAgent) {
          // Not the last agent - show next agent in sequence
          const nextAgentFull = agents[currentIndex + 1];
          const nextAgent = nextAgentFull.includes('/') ? nextAgentFull.split('/')[1] : nextAgentFull;
          return `If complete: send to \`${nextAgent}\``;
        } else {
          // Last agent - route back to requester
          return 'If complete: send to `{{requester}}`';
        }
      }

      // For iterative workflow, add rejection routing
      if (isIterative) {
        let instructions = '';
        ...
        if (!isLastAgent) {
          ...
          const nextAgentFull = agents[currentIndex + 1];
          const nextAgent = nextAgentFull.includes('/') ? nextAgentFull.split('/')[1] : nextAgentFull;
          instructions += `If approved: send to \`${nextAgent}\``;
        } else {
          // Last agent - route back to requester
          instructions += 'If complete: send to `{{requester}}`';
        }

        // Add rejection routing if not first agent
        if (currentIndex > 0) {
          const prevAgentFull = agents[currentIndex - 1];
          const prevAgent = prevAgentFull.includes('/') ? prevAgentFull.split('/')[1] : prevAgentFull;
          instructions += `\nIf rejected: send to \`${prevAgent}\``;
        }
      ...
      Logger.warn('prompt-builder', `Failed to generate next agent instructions: ${error.message}`);
      return 'Process and complete task.';
    }
  }
