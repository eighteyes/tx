...
class Message {
  ...
  static send(toAgent, task, context = '', metadata = {}) {
    ...
    const fromAgent = metadata.from ?
      (metadata.from.includes('/') ? metadata.from.split('/')[1] : metadata.from) : 'user';
    const toAgentOnly = toAgent.includes('/') ? toAgent.split('/')[1] : toAgent;
...
${context}`;
  ...
  static validateFrontmatter(metadata) {
    ...
    const validTypes = ['task', 'task-complete', 'ask', 'ask-response', 'update', 'prompt'];
