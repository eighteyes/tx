...
class Queue {
  ...
  static routeMessage(sourceMesh, sourceAgent, message, filepath) {
    ...
    // Validate routing against mesh config (if enabled)
    if (ConfigLoader.isFeatureEnabled('beta.routing_validation')) {
      // Validate completion_agent for task-complete messages
    ...
    try {
      ...
      // Record that this message was delivered to this agent (prevents re-delivery, if enabled)
      if (ConfigLoader.isFeatureEnabled('beta.delivery_tracking')) {
