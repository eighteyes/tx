...
class Queue {
  ...
  static validateRouting(message, sourceMesh, sourceAgent) {
    try {
      ...
      // Load mesh config
      const configPath = `meshes/mesh-configs/${sourceMesh}.json`;
      if (!fs.existsSync(configPath)) {
        return { valid: true }; // No config = no validation
      }

      const config = fs.readJsonSync(configPath);
      const routing = config.routing?.[sourceAgent];
      ...
      const statusRoutes = routing[status];
      if (!statusRoutes) {
        return {
          valid: false,
          error: `Unknown status "${status}" for agent ${sourceAgent}. Valid statuses: ${Object.keys(routing).join(', ')}`
        };
      }

      // Extract target agent from 'to' field
      const targetAgent = to.includes('/') ? to.split('/')[1] : to;
      ...
      Logger.warn('queue', `Routing validation error: ${error.message}`);
