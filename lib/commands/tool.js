 ...
 * Uses @lb-project/know package (local file dependency or published version)
 */
function executeKnow(args) {
  ...
  try {
    ...
    const knowPackage = require.resolve('@lb-project/know/package.json');
    ...
    const fallbackPaths = [
      path.join(process.cwd(), 'node_modules', '@lb-project', 'know', 'bin', 'know.js'),
      path.join(__dirname, '..', '..', 'node_modules', '@lb-project', 'know', 'bin', 'know.js'),
    ...
    if (!knowPath) {
      console.error('Error: @lb-project/know package not found');
