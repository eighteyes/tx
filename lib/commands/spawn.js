...
const { SimpleDirectoryInitializer } = require('../simple-directory-initializer');
...
async function spawn(mesh, agent = null, options = {}) {
  ...
  try {
    // Core mesh doesn't get a UUID (it's the orchestrator)
    ...
    if (!agent) {
      ...
      for (const agentName of agentsToSpawn) {
        await spawnSingleAgent(mesh, meshInstance, agentName, options);
      }
      return;
    }

    // If agent is specified, spawn just that one
    await spawnSingleAgent(mesh, meshInstance, agent, options);
  } catch (error) {
...
async function spawnSingleAgent(mesh, meshInstance, agent, options = {}) {
  // Declare variables outside try-catch so they're accessible in both blocks
  let taskUID = '';
  let finalAgentName = agent;
  ...
  try {
    ...
    // Generate UID suffix from task if provided (deprecated - now using mesh instance UUID)
    ...
    // Special case: core/core should be just "core"
    ...
    SimpleDirectoryInitializer.initializeAgentDirectories(meshInstance, finalAgentName);
    ...
    const sessionCreated = TmuxInjector.createSession(sessionName, 'bash', true, meshInstance, finalAgentName);
    if (!sessionCreated) {
    ...
    safeConsoleLog(`   TX_MESH=${meshInstance}, TX_AGENT=${finalAgentName}`);
    ...
    SimpleQueue.processQueueBacklog(meshInstance);
    ...
    safeConsoleLog(`   Kill: tx kill ${meshInstance} ${agentName}\n`);
