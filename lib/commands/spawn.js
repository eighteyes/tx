...
const { EventLogManager } = require('../event-log-manager');
...
/**
 * Find the Claude binary path by checking multiple locations
 * Tries in order:
 * 1. /usr/local/share/npm-global/bin/claude
 * 2. ~/.claude/local/claude
 * 3. claude in PATH (via which) - as last resort
 * @returns {string} Path to Claude binary
 * @throws {Error} If Claude binary is not found in any location
 */
function findClaudePath() {
  ...
  // Check each hardcoded path first
  ...
  try {
    const whichResult = execSync('which claude', { encoding: 'utf8', stdio: ['pipe', 'pipe', 'ignore'] }).trim();
  ...
  // If we get here, Claude was not found anywhere
  throw new Error(
    'Claude binary not found. Tried:\n' +
    possiblePaths.map(p => `  - ${p}`).join('\n') +
    '\n  - claude in PATH (last resort)\n\n' +
    'Please install Claude or set up the correct path.'
  );
}
...
async function spawn(mesh, agent = null, options = {}) {
  ...
  try {
    ...
    if (!agent) {
      ...
      // Calculate timeout based on number of agents (base 30s + 15s per agent)
      const agentCount = agentsToSpawn.length;
      const timeoutPerAgent = 30000 + (agentCount * 15000);
      const timeoutSeconds = Math.floor(timeoutPerAgent / 1000);

      if (agentCount > 1) {
        safeConsoleLog(`⏱️  Spawning ${agentCount} agents with ${timeoutSeconds}s timeout per agent\n`);
      }

      // Spawn all agents sequentially with the same mesh instance ID
      for (const agentName of agentsToSpawn) {
        await spawnSingleAgent(mesh, meshInstance, agentName, options, isPersistent, timeoutPerAgent);
      }

      // Force exit after all spawns complete (unless keepAlive is set)
    ...
    // If agent is specified, spawn just that one (use default timeout)
    await spawnSingleAgent(mesh, meshInstance, agent, options, isPersistent);

    // Force exit after spawn completes (don't wait for event loop to drain)
    // This prevents spawn from hanging when called from CLI
    // BUT: Don't exit if called from tx start (keepAlive option)
    if (!options.keepAlive) {
      setImmediate(() => process.exit(0));
    }
  } catch (error) {
...
async function spawnSingleAgent(mesh, meshInstance, agent, options = {}, isPersistent = false, timeout = 30000) {
  // Declare variables outside try-catch so they're accessible in both blocks
  let taskUID = '';
  ...
  try {
    ...
    const agentId = `${meshInstance}/${finalAgentName}`;
    await StateManager.initializeAgent(agentId, sessionName);
    await StateManager.transitionState(agentId, StateManager.STATES.INITIALIZING);

    // Start Claude in the session
    // Use full path to avoid alias issues in tmux sessions
    safeConsoleLog('🤖 Starting Claude in session...');
    const claudePath = findClaudePath();
    TmuxInjector.send(sessionName, `${claudePath} --dangerously-skip-permissions`);
    TmuxInjector.send(sessionName, 'Enter');

    // Wait for Claude to be ready
    safeConsoleLog('⏳ Waiting for Claude to initialize...');
    const readyStatus = await TmuxInjector.claudeReadyCheck(sessionName, timeout);
    ...
    // Inject agent options if configured (BEFORE prompt injection)
    ...
    // Build and inject agent prompt directly (not via event log)
    safeConsoleLog('📝 Building agent prompt...');
    const prompt = PromptBuilder.build(mesh, meshInstance, finalAgentName, options.init || null);

    // Write prompt to temp file and inject via @filepath syntax
    const promptTempPath = path.join('.ai/tx/temp', `prompt-${agentId.replace('/', '-')}.md`);
    fs.ensureDirSync(path.dirname(promptTempPath));
    fs.writeFileSync(promptTempPath, prompt);

    safeConsoleLog('💉 Injecting prompt to agent...');
    TmuxInjector.injectFile(sessionName, promptTempPath, true); // isPrompt=true
    Logger.log('spawn', 'Initial prompt injected', { agentId, promptFile: promptTempPath });

    // Wait for prompt injection to complete
    safeConsoleLog('⏳ Waiting for prompt injection to complete...');
    await new Promise(resolve => setTimeout(resolve, 3000)); // Wait for injection queue to process
    const promptIdle = await TmuxInjector.waitForIdle(sessionName, 2000, 30000);
    if (!promptIdle) {
      Logger.warn('spawn', 'Session may not be idle after prompt injection', { agentId });
    }
    safeConsoleLog('✅ Prompt injected\n');
    ...
    Logger.log('spawn', `Agent ${agentId} marked as ready`);

    // NOTE: Consumer is now managed by ConsumerDaemon (started by SystemManager)
    // The daemon monitors mesh_states table and starts/stops consumers automatically

    // Update mesh instance state in SQLite
    const db = StateDB.init();
    const now = Date.now();
    db.prepare(`
      INSERT OR REPLACE INTO mesh_states (
        mesh_name, status, current_agent, started_at, updated_at, metadata
      ) VALUES (?, ?, ?, ?, ?, ?)
    `).run(
      meshInstance,
      'active',
