...
async function spawn(mesh, agent = null, options = {}) {
  ...
  try {
    ...
    if (!agent) {
      ...
      // Spawn all agents sequentially
      for (const agentName of agentsToSpawn) {
        await spawnSingleAgent(mesh, agentName, options);
      }
      return;
    }

    // If agent is specified, spawn just that one
    await spawnSingleAgent(mesh, agent, options);
  } catch (error) {
...
async function spawnSingleAgent(mesh, agent, options = {}) {
  // Declare variables outside try-catch so they're accessible in both blocks
  let taskUID = '';
  let finalAgentName = agent;
  ...
  try {
    ...
    SimpleDirectoryInitializer.initializeAgentDirectories(mesh, finalAgentName);
    ...
    const sessionCreated = TmuxInjector.createSession(sessionName, 'bash', true, mesh, finalAgentName);
    if (!sessionCreated) {
    ...
    SimpleQueue.processQueueBacklog(mesh);
