...
async function logsInteractive(options = {}) {
  ...
  function getFilteredLogs() {
    ...
    // If any filters are active, show only matching components
  ...
  function displayHeader() {
    ...
    const mode = activeFilters.size > 0
      ? `${colors.dim}[filtered]${colors.reset}`
      : `${colors.dim}[all]${colors.reset}`;

    console.log(`${colors.dim}tx logs ${colors.reset}${filterDisplay}  ${colors.green}a${colors.reset}${colors.dim} clear${colors.reset}  ${colors.dim}c clear-logs  q quit ${mode}${colors.reset}\n`);
  }
