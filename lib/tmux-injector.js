...
class TmuxInjector {
  ...
  /**
   * Inject plain text directly into tmux session
   * Used for events that don't need file references (nudges, reminders, errors)
   * @param {string} session - Session name
   * @param {string} text - Text to inject
   * @param {object} options - Options: { wrap: boolean }
   * @returns {boolean} Success
   */
  static injectText(session, text, options = {}) {
    // Add to injection queue
    TmuxInjector._enqueueInjection(session, 'text', { text, options });
    return true;
  }
  ...
  static _doInjectFile(session, data) {
    try {
      ...
      // Prepend instruction text for prompts and task messages
      // This helps agents understand they need to ACT on the message, not just acknowledge it
      const injectionText = isPrompt
        ? `Read and follow instructions: @${absolutePath.replace(/'/g, "'\\''")}`
        : `Read and act on this message: @${absolutePath.replace(/'/g, "'\\''")}`;
  ...
  /**
   * Actually perform text injection (internal method)
   * @param {string} session - Session name
   * @param {object} data - Injection data { text, options }
   */
  static _doInjectText(session, data) {
    try {
      const { text, options = {} } = data;

      // Use tmux paste buffer for text to handle special characters
      const bufferName = '_text_buffer';

      // Escape text for shell
      const escapedText = text.replace(/'/g, "'\\''");

      // Set buffer and paste
      execSync(`tmux set-buffer -b ${bufferName} '${escapedText}'`, { stdio: 'pipe' });
      execSync(`tmux paste-buffer -t ${session} -b ${bufferName}`, { stdio: 'pipe' });
      TmuxInjector._sleep(300);

      // Send Enter to submit
      execSync(`tmux send-keys -t ${session} Enter`, { stdio: 'pipe' });
      TmuxInjector._sleep(300);

      Logger.log('tmux-injector', `Text injected: ${text.substring(0, 50)}...`, {
        session,
  ...
  static _enqueueInjection(session, type, data) {
    ...
    // Only log "Queued" if there's actually a queue (already processing or multiple items)
    // If queue length is 1 and not processing, it will be processed immediately
    if (queueState.processing || queueState.queue.length > 1) {
      Logger.log('tmux-injector', `Queued ${type} injection`, {
        session,
        seq: injection.sequenceNumber,
        queueLen: queueState.queue.length
      });
    }
  ...
  static async _processInjectionQueue(session) {
    ...
    Logger.log('tmux-injector', `Processing ${injection.type} injection`, {
      session,
      seq: injection.sequenceNumber,
    ...
    setTimeout(async () => {
      try {
        ...
        if (!isIdle) {
          Logger.warn('tmux-injector', 'Idle timeout', { session });
        }
      } catch (error) {
  ...
  static createSession(session, command = null, loadConfig = true) {
    try {
      ...
      const configLoaded = loadConfig ? TmuxInjector.loadConfig(session) : false;

      Logger.log('tmux-injector', 'Session created', {
        session,
        config: configLoaded ? 'loaded' : 'skipped'
      });
  ...
  static loadConfig(session, configPath = '.tmux.conf') {
    try {
      ...
      // Don't log separately - will be logged as part of createSession
      return true;
    } catch (error) {
  ...
  /**
   * Detach from a tmux session (leave it running)
   */
  static detachSession(session) {
    try {
      // Send detach key sequence to any clients attached to this session
      execSync(`tmux detach-client -s ${session}`, { stdio: 'pipe' });
      Logger.log('tmux-injector', 'Session detached (still running)', { session });
      return true;
    } catch (error) {
      Logger.warn('tmux-injector', `Session not found or no clients attached: ${session}`);
      return false;
    }
  }
    ...
    while (Date.now() - startTime < timeout) {
      try {
        ...
        if (output.includes('⏵⏵ bypass permissions on') &&
          ...
          Logger.log('tmux-injector', 'Claude ready', {
            session,
            waitMs: waitTime
          });
  ...
  static async waitForIdle(session, idleTime = 1000, timeout = 10000, pollInterval = 200) {
    ...
    while (Date.now() - startTime < timeout) {
      try {
        ...
        // Normalize output by stripping animated content that changes frequently
        const normalizedOutput = TmuxInjector._normalizeOutput(output);

        // Check if output has changed
        if (normalizedOutput !== lastOutput) {
        ...
        if (idleDuration >= idleTime) {
          // Don't log idle success - only log failures/warnings
          return true;
        }
  ...
  /**
   * Normalize output by stripping animated content
   * Removes Claude Code status animations that change frequently
   */
  static _normalizeOutput(output) {
    return output
      // Strip "Lollygagging" animation with timer
      .replace(/✻ Lollygagging… \(esc to interrupt · \d+s · [^\)]+\)/g, '✻ Lollygagging…')
      // Strip "Thinking" animation with timer and token count
      .replace(/∴ Thinking… \(esc to interrupt · \d+s · [^\)]+\)/g, '∴ Thinking…')
      // Strip generic timer patterns (X · Ys · token info)
      .replace(/\(esc to interrupt · \d+s · [^\)]+\)/g, '(esc to interrupt)')
      // Strip status line animations with changing token counts
      .replace(/⏵⏵ bypass permissions on \(shift\+tab to cycle\)\s+Thinking on \(tab to toggle\)\s*\n\s*\d+ tokens/g, '⏵⏵ bypass permissions on (shift+tab to cycle)    Thinking on (tab to toggle)')
      // Strip standalone changing token counts at the end of lines
      .replace(/\s+\d+ tokens\s*$/gm, '');
  }
...
/**
 * Instance wrapper for simple test/use cases
 */
class TmuxInjectorInstance {
  constructor() {}
  ...
  injectFile(session, filepath, isPrompt = false) {
    return TmuxInjector.injectFile(session, filepath, isPrompt);
  }
}

module.exports = TmuxInjectorInstance;
module.exports.TmuxInjector = TmuxInjector;
