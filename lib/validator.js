...
class Validator {
  ...
  static validateMeshConfig(meshName) {
    ...
    try {
      const configPath = path.join('meshes', 'mesh-configs', `${meshName}.json`);
      ...
      try {
        ...
        Evidence.record(
          ...
          error,
          {
            mesh: meshName,
            component: 'validator',
            additional: { configPath, error: parseError.stack }
          }
        );
      ...
      if (config.agents) {
        config.agents.forEach(agentSpec => {
          ...
          const agentConfigPath = path.join('meshes', 'agents', agentSpec, 'config.json');
          ...
          if (!fs.existsSync(agentConfigPath)) {
            ...
            Evidence.record(
              Evidence.Types.CONFIG_INVALID,
              `Agent config not found for '${agentSpec}'`,
              {
                mesh: meshName,
                agent: agentName,
                component: 'validator',
                additional: { agentConfigPath, agentSpec }
              }
            );
          }
        });
      ...
      if (errors.length > 0) {
        Evidence.record(
          Evidence.Types.VALIDATION_FAILURE,
          `Mesh config validation failed for '${meshName}': ${errors.length} errors`,
          {
            mesh: meshName,
            component: 'validator',
            additional: { errors, warnings, configPath }
          }
        );
      }
  ...
  static validateAllMeshConfigs() {
    ...
    try {
      const meshConfigsDir = path.join('meshes', 'mesh-configs');
  ...
  static validateSystem() {
    ...
    if (!results.valid) {
      Evidence.record(
        Evidence.Types.VALIDATION_FAILURE,
        `System validation failed: ${results.meshConfigs.totalErrors} errors across ${Object.keys(results.meshConfigs.meshes).length} meshes`,
        {
          component: 'validator',
          additional: {
            totalErrors: results.meshConfigs.totalErrors,
            totalWarnings: results.meshConfigs.totalWarnings,
            orphanedCount: results.orphanedMessages.orphans.length
          }
        }
      );
    }
