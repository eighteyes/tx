...
const { E2ELogger } = require('./e2e-logger');
...
class E2EWorkflow {
  constructor(meshName, agentName, initialTask = null, options = {}) {
    ...
    // Initialize logger
    this.logger = new E2ELogger(options.logFilePath || '.ai/tx/logs/e2e-test.log');
    this.logger.info(`Initialized E2EWorkflow for ${meshName}/${agentName}`, {
      initialTask: this.initialTask,
      workflowTimeout: this.workflowTimeout,
      hitlEnabled: this.hitl.enabled
    });
  }
  ...
  async test() {
    ...
    this.logger.info(`Starting workflow test: ${this.meshName}/${this.agentName}`);
    ...
    try {
      ...
      this.logger.info('Waiting for core session to be idle');
      ...
      if (!coreIdle) {
        ...
        this.logger.warn('Core session may not be idle, but continuing');
      } else {
        console.log('✅ Core session is idle\n');
        this.logger.success('Core session is idle');
      }
      ...
      this.logger.step(2, 'Sending natural language instruction to Claude');
      this.logger.info('Instruction', { task: this.initialTask });
      ...
      if (!results.step2MeshSpawned) {
        ...
        this.logger.error('Expected session pattern not found', {
          expectedPattern,
          meshSessions,
          allSessions
        });
      ...
      this.logger.success('Mesh spawned', { sessionName: this.meshSession });
      ...
      this.logger.step(3, 'Checking if mesh received message');
      ...
      if (!results.step3MeshReceivedMessage) {
        ...
        this.logger.error('Mesh did not receive message');
        return false;
      }
      console.log('✅ Mesh received message\n');
      this.logger.success('Mesh received message');
      ...
      this.logger.step(4, 'Checking if mesh sent response back to core');
      ...
      if (!results.step4MeshRespondedToCore) {
        ...
        this.logger.error('Mesh did not send response to core');
        return false;
      }
      console.log('✅ Mesh sent response to core\n');
      this.logger.success('Mesh sent response to core');
      ...
      this.logger.step(5, 'Checking if core received response from mesh');
      ...
      if (!results.step5CoreReceivedResponse) {
        ...
        this.logger.error('Core did not receive response from mesh');
        return false;
      }
      console.log('✅ Core received response from mesh\n');
      this.logger.success('Core received response from mesh');

      console.log('✅ All workflow tests PASSED\n');
      this.logger.success('All workflow tests PASSED', results);
      ...
      this.logger.endSession(true, testDuration);
      return true;
    } catch (error) {
      console.error('❌ Workflow error:', error.message);
      this.logger.error('Workflow error', { message: error.message, stack: error.stack });
      const testDuration = Date.now() - testStartTime;
      this.logger.endSession(false, testDuration);
      return false;
    }
  }
  ...
  async _checkCoreReady() {
    try {
      ...
      if (!sessions.includes(this.coreSession)) {
        this.logger.warn('Core session not found', { sessions });
        return false;
      }

      // Check if Claude is ready in the session
      const ready = await TmuxInjector.claudeReadyCheck(this.coreSession, 5000);
      if (!ready) {
        this.logger.warn('Claude not ready in core session');
      }
      return ready;
    } catch (e) {
      this.logger.error('Error checking core ready', { error: e.message });
      return false;
    }
  }
  ...
  async _waitForMeshSpawn(maxWaitTime = 20000, pollInterval = 500) {
    ...
    while (Date.now() - startTime < maxWaitTime) {
      try {
        ...
        // Get sessions with their creation timestamps
        ...
        // IMPORTANT: Find the MOST RECENTLY created session (highest timestamp)
        // to avoid matching sessions from previous test runs
        let mostRecentSession = null;
        let mostRecentTime = 0;

        for (const line of sessions) {
          const [sessionName, timestamp] = line.split(' ');
          const time = parseInt(timestamp, 10);

          const pattern = new RegExp(`^${this.meshName}-[0-9a-f]{6}-${this.agentName}$`);
          if (pattern.test(sessionName) && time > mostRecentTime) {
            mostRecentTime = time;
        ...
        if (mostRecentSession) {
          console.log(`   Found session: ${mostRecentSession}`);
          this.logger.info('Found mesh session', { sessionName: mostRecentSession });
          this.meshSession = mostRecentSession;
          return true;
        }
        ...
        if (sessions.includes(legacySession)) {
          ...
          this.logger.info('Found legacy mesh session', { sessionName: legacySession });
          this.meshSession = legacySession;
          return true;
        }
      } catch (e) {
        this.logger.debug('Error checking for mesh session', { error: e.message });
      }

      await new Promise(resolve => setTimeout(resolve, pollInterval));
    }

    this.logger.warn('Mesh session spawn timeout', { meshName: this.meshName, agentName: this.agentName });
    return false;
  }

  /**
   * Check if mesh received message by verifying actual file delivery
   * Validates that a task message was delivered to the agent via centralized event log
   */
  async _checkMeshReceivedMessage(maxWaitTime = 30000, pollInterval = 1000) {
    if (!this.meshSession) return false;

    const startTime = Date.now();
    // Extract mesh instance ID by removing the last occurrence of -agentName
    // E.g., "test-echo-6c135f-echo" -> "test-echo-6c135f"
    const suffix = `-${this.agentName}`;
    const meshInstanceId = this.meshSession.endsWith(suffix)
      ? this.meshSession.substring(0, this.meshSession.length - suffix.length)
      : this.meshSession;

    // NEW: Messages are in centralized .ai/tx/msgs/ directory
    const centralMsgsDir = `.ai/tx/msgs`;

    while (Date.now() - startTime < maxWaitTime) {
      try {
        // Step 1: Check centralized message directory for message from core to this agent
        ...
        // Look for message with pattern: {mmddhhmmss}-{type}-{from-agent}>{to-agent}-{msgId}.md
        // Filenames use only agent names (part after /), not full mesh/agent paths
        // Example: 1102083000-task-core>ping-abc123.md
        const expectedPattern = new RegExp(`^\\d{10}-(task|ask)-core>${this.agentName}-.*\\.md$`);
        ...
        // 1. The filename appears in the output
        // 2. The full file path appears (Read tool output)
        // 3. The injection text "Read and act on this message:" with the path
        // 4. The Read() tool call format with the msgs directory
        const fullPath = path.join(centralMsgsDir, messageFilename);
        if (sessionOutput.includes(messageFilename) ||
            sessionOutput.includes(fullPath) ||
            sessionOutput.includes(`Read and act on this message: @`) ||
            sessionOutput.includes(`Read and follow instructions: @`) ||
            (sessionOutput.includes(`Read `) && sessionOutput.includes(centralMsgsDir)) ||
            (sessionOutput.includes(`Read(`) && sessionOutput.includes(centralMsgsDir))) {
          console.log(`   ✅ Message delivery validated:`);
          console.log(`      - Centralized message: ${messageFilename}`);
          console.log(`      - Target session shows file injection`);
          this.logger.messageInfo('Delivered', 'core', this.agentName, {
        ...
        this.logger.debug('Error checking message delivery', { error: e.message });
      }
    ...
    console.log(`      - Central msgs dir: ${centralMsgsDir}`);
    this.logger.error('Message routing failed', { centralMsgsDir, meshSession: this.meshSession });
    return false;
  }

  /**
   * Check if mesh sent response back to core by checking for task-complete message
   * Looks in centralized msgs directory for response message from agent to core
   */
  async _checkMeshRespondedToCore(maxWaitTime = 30000, pollInterval = 1000) {
    ...
    while (Date.now() - startTime < maxWaitTime) {
      try {
        ...
        // Look for task-complete message from agent to core
        // Pattern: {mmddhhmmss}-task-complete-{agentName}>core-{msgId}.md
        const responsePattern = new RegExp(`^\\d{10}-task-complete-${this.agentName}>core-.*\\.md$`);

        for (const file of msgFiles) {
          if (responsePattern.test(file)) {
            const fullPath = path.join(centralMsgsDir, file);
            const content = fs.readFileSync(fullPath, 'utf-8');

            // Verify it's a valid response to core with task-complete type
            if ((content.includes('to: core') || content.includes('to: core/core')) &&
                (content.includes('type: task-complete') || content.includes('task-status'))) {
              console.log(`   ✅ Mesh sent response to core: ${file}`);
              this.logger.messageInfo('Task-complete', this.agentName, 'core', { messageFile: file });
              return true;
            }
          }
        }
      } catch (e) {
        this.logger.debug('Error checking mesh response', { error: e.message });
      }

      await new Promise(resolve => setTimeout(resolve, pollInterval));
    }

    console.log(`   ⚠️  No task-complete message from ${this.agentName} to core found in ${centralMsgsDir}`);
    this.logger.warn(`No task-complete message from ${this.agentName} to core`, { centralMsgsDir });
    return false;
  }

  /**
   * Check if core received response by verifying task-complete message exists
   * Checks centralized msgs directory for response from agent to core
   */
  async _checkCoreReceivedResponse(maxWaitTime = 30000, pollInterval = 1000) {
    ...
    while (Date.now() - startTime < maxWaitTime) {
      try {
        ...
        // Look for any task-complete message to core from this agent
        // Pattern: {mmddhhmmss}-task-complete-{agentName}>core-{msgId}.md
        const responsePattern = new RegExp(`^\\d{10}-task-complete-${this.agentName}>core-.*\\.md$`);
        ...
        for (const file of msgFiles) {
          if (responsePattern.test(file)) {
            ...
            // Verify the message is to core and has task-complete type
            if ((content.includes('to: core') || content.includes('to: core/core')) &&
                content.includes('type: task-complete')) {
              console.log(`   ✅ Core received response from ${this.agentName}: ${file}`);
              return true;
            }
          }
        }
    ...
    console.log(`   ⚠️  Did not find task-complete response from ${this.agentName} to core`);
    return false;
  }
  ...
  async _handleHITL() {
    ...
    const meshInstanceId = this.meshSession.endsWith(suffix)
      ? this.meshSession.substring(0, this.meshSession.length - suffix.length)
      : this.meshSession;
    // NEW: Watch centralized message directory
    const centralMsgsDir = `.ai/tx/msgs`;
    ...
    console.log(`📍 HITL mode: watching ${centralMsgsDir} for ask messages\n`);
    console.log(`   Max questions: ${maxQuestions}\n`);

    while (this.hitlState.questionCount < maxQuestions) {
      // Wait for new ask message
      const askFile = await this._waitForAskMessage(centralMsgsDir, questionTimeout, meshInstanceId);
      ...
      const askPath = path.join(centralMsgsDir, askFile);
      const askContent = fs.readFileSync(askPath, 'utf-8');

      // Extract msg-id from filename pattern: {mmddhhmmss}-ask-{from}>{to}-{msgId}.md
      // msgId is the last part after the last dash
      const msgIdMatch = askFile.match(/^\d{10}-ask-[^>]+>[^-]+-(.+)\.md$/);
      const msgId = msgIdMatch ? msgIdMatch[1] : `hitl-q-${this.hitlState.questionCount}`;

      // Find matching response
      const responseText = this._findMatchingResponse(askContent);

      // Create ask-response message in centralized directory
      ...
      // Response filename uses only agent names (part after /)
      const responseFileName = `${timestamp}-ask-response-${this.agentName}>core-${msgId}.md`;
      const responseFilePath = path.join(centralMsgsDir, responseFileName);
...
`;
  ...
  /**
   * Wait for a new ask message to appear in the centralized msgs directory
   */
  async _waitForAskMessage(centralMsgsDir, timeout = 45000, meshInstanceId) {
    const startTime = Date.now();
    ...
    while (Date.now() - startTime < timeout) {
      try {
        ...
        const files = fs.readdirSync(centralMsgsDir).filter(f => f.endsWith('.md'));

        // Look for ask messages from this agent to core
        // Pattern: {mmddhhmmss}-ask-{from-agent}>{to-agent}-{msgId}.md
        // Example: 1102083000-ask-ping>core-abc123.md
        const askPattern = new RegExp(`^\\d{10}-ask-${this.agentName}>core-.*\\.md$`);
        ...
        for (const file of files) {
          ...
          // Check if filename matches ask message pattern
          if (!askPattern.test(file)) {
            continue;
          }
          ...
          // Verify it's an ask message to core
          if (content.includes('type: ask') &&
              content.includes('to: core/core') &&
              content.includes(`from: ${meshInstanceId}/${this.agentName}`)) {
            return file;
          }
        }
      } catch (e) {
