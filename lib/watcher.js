...
class Watcher {
   ...
   *   - .ai/tx/mesh/[mesh-name]/msgs/{inbox,next,active,outbox}/*.md
   *   - .ai/tx/mesh/[mesh-name]/agents/[agent]/msgs/{inbox,next,active,outbox}/*.md
   */
  static start() {
    if (Watcher.watching) {
    ...
    try {
      // Only watch active message queue directories (no ** to avoid watching parent directories)
      // This pattern matches:
      //   - .ai/tx/mesh/*/msgs/{inbox,next,active,outbox}/*.md (mesh-level)
      //   - .ai/tx/mesh/*/agents/*/msgs/{inbox,next,active,outbox}/*.md (agent-level)
      const watchPatterns = [
        '.ai/tx/mesh/*/msgs/{inbox,next,active,outbox}/*.md',
        '.ai/tx/mesh/*/agents/*/msgs/{inbox,next,active,outbox}/*.md'
      ];
      ...
      Watcher.watcher = chokidar.watch(watchPatterns, {
        persistent: true,
        ignoreInitial: false,
        awaitWriteFinish: {
          stabilityThreshold: 100,
        ...
        interval: 100,
        binaryInterval: 300,
        // Only watch message queue directories, ignore all parent/sibling directories
        // This prevents watching:
        //   - .ai/tx/
        //   - .ai/tx/mesh/
        //   - .ai/tx/mesh/<meshname>/
        //   - .ai/tx/mesh/<meshname>/agents/
        // And only watches files under:
        //   - .ai/tx/mesh/*/msgs/{inbox,next,active,outbox}/*.md
        //   - .ai/tx/mesh/*/agents/*/msgs/{inbox,next,active,outbox}/*.md
        ignored: (path) => {
          // Must contain /msgs/ to be watched
          if (!path.includes('/msgs/')) return true;
          // Must be in a valid queue directory
          if (!/(\/msgs\/(inbox|next|active|outbox))/.test(path)) return true;
          // Don't ignore this path
          return false;
        }
      });
